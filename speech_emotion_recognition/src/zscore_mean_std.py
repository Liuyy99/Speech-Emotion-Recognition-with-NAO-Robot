...
Description: This script calculates the mean and standard deviation of "neutral" speech's 3-d
mel-spectrogram (in vector representation) for z-score normalization. The means and standard
deviations of all training corpora (4 in total) are calculated and stored.
"""
...
def add_to_list(train_nums, dataset_index, combined_train_data, static1, delta11, delta21):
    train_num = train_nums[dataset_index]
    combined_train_data[dataset_index]["mel"][train_num * 300:(train_num + 1) * 300] = static1
    combined_train_data[dataset_index]["d_mel"][train_num * 300:(train_num + 1) * 300] = delta11
    combined_train_data[dataset_index]["dd_mel"][train_num * 300:(train_num + 1) * 300] = delta21
...
def read_combined_dataset():
    ...
    output = './zscore_4_datasets' + str(filter_num) + '.pkl'
    f = open(output, 'wb')
